...
export class ResponsibilityService {
  ...
  {
    return this.httpClient.get(`${BASE_API_URL}/employee/${id}`, getAuthorizationHeader(token));
  }

  getByProjectAndTeam({teamId, projectId, token}: {teamId: number, projectId: number, token: string})
  {
    return this.httpClient.get(`${BASE_API_URL}/project/${projectId}/team/${teamId}`, getAuthorizationHeader(token));
  }

  add({responsibility, token}: {responsibility: CreateResponsibilityDto, token: string})
  {
    return this.httpClient.post(`${BASE_API_URL}`, {
      ...responsibility,
